...
const ConfirmOrder = () => {
  ...
  const coursesPrice = userCart.reduce(
    (acc, item) => acc + item.tuition,
    0
  );
  const shippingPrice = coursesPrice > 200 ? 30 : 60;
  const taxPrice = Number((0.05 * coursesPrice).toFixed(2));
  const totalPrice = (coursesPrice + shippingPrice + taxPrice).toFixed(2);

  const handlePlaceOrder = () => {

    const courseIds = userCart.map((item) => item._id);

    // Create an object with order details
    const orderData = {
      user: userId,
      courseIds, // Assuming you have a field in your Course model to store the Course ID
